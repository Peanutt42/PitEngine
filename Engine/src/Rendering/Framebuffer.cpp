...
namespace Pit::Rendering {
	...
	namespace Utils {
		...
		static GLenum HazelFBTextureFormatToGL(FramebufferTextureFormat format) {
			...
			PIT_ENGINE_ASSERT(Rendering, false, "");
			return 0;
		}
	...
	void Framebuffer::Invalidate() {
		...
		if (m_ColorAttachments.size() > 1) {
			PIT_ENGINE_ASSERT(Rendering, m_ColorAttachments.size() <= 4, "");
		...
		PIT_ENGINE_ASSERT(Rendering, glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	void Framebuffer::Resize(uint32_t width, uint32_t height) {
		if (width == 0 || height == 0 || width > s_MaxFramebufferSize || height > s_MaxFramebufferSize) {
			PIT_ENGINE_WARN(Rendering, "Attempted to rezize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	int Framebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y) {
		PIT_ENGINE_ASSERT(Rendering, attachmentIndex < m_ColorAttachments.size(), "");
